...
export const chipsReducers = {
    incrementBet: (state, action) => {
        const newBet = state.bet + action.payload.value;
        if (newBet > state.bankroll) return; // Don't exceed bankroll
        
        const LIMIT = 20;
        state.bet = newBet;
        const newIdx = (state.chips.slice(-1)[0]?.order ?? 0) + 1;
        ...
        if (state.chips.length < LIMIT) {
            state.chips.push({
                ...
                order: newIdx,
                style: {
                    bottom: `${state.chips.length * 4}px`,
