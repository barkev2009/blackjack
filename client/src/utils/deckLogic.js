...
export const canSplit = (hand, playerBalance, bet) => {
    return hand.length === 2 &&
        getCardValueForSplit(hand[0]) === getCardValueForSplit(hand[1]) &&
        playerBalance >= bet;
};

// Получить значение карты для сплита (10, J, Q, K считаются одинаковыми)
const getCardValueForSplit = (card) => {
    if (['10', 'J', 'Q', 'K'].includes(card.value)) return '10';
    return card.value;
};

// Создать две руки из сплита
export const createSplitHands = (originalHand, deck, bet) => {
    const newDeck = [...deck];
    const hand1 = [originalHand[0]];
    const hand2 = [originalHand[1]];

    // Берем по карте для каждой руки
    const { card: card1, newDeck: deck1 } = drawCard(newDeck);
    ...
    return {
        hands: [hand1, hand2],
        newDeck: deck2,
        newBet: bet * 2 // Ставка удваивается
    };
};
...
export const getBasicStrategyAdvice = (playerHand, dealerCard) => {
    ...
    if (playerHand.length === 2 && getCardValueForSplit(playerHand[0]) === getCardValueForSplit(playerHand[1])) {
...
// Получить рекомендацию для сплита
export const getSplitAdvice = (playerCard, dealerCard) => {
    const playerValue = getCardValueForSplit({ value: playerCard });
    const dealerValue = getDealerValue(dealerCard);

    const splitRules = {
        'A': 'Всегда сплит',
        '10': 'Никогда не сплит',
        '9': dealerValue >= 7 && dealerValue <= 10 ? 'Не сплит' : 'Сплит',
        '8': 'Всегда сплит',
        '7': dealerValue <= 7 ? 'Сплит' : 'Не сплит',
        '6': dealerValue <= 6 ? 'Сплит' : 'Не сплит',
        '5': 'Никогда не сплит',
        '4': dealerValue >= 5 && dealerValue <= 6 ? 'Сплит' : 'Не сплит',
        '3': dealerValue <= 7 ? 'Сплит' : 'Не сплит',
        '2': dealerValue <= 7 ? 'Сплит' : 'Не сплит'
    };

    return splitRules[playerValue] || 'Не сплит';
};